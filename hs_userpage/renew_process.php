...
// Update highschool_account with semester and year_level
if (!empty($semester) || !empty($year_level)) {
    ...
    if (!empty($updates)) {
        ...
        $update_sql = "UPDATE highschool_account SET " . implode(", ", $updates) . " WHERE applicant_id = ?";
...
if (!empty($_FILES['renewal_documents']['name'][0])) {
    foreach ($_FILES['renewal_documents']['name'] as $i => $name) {
        $tmp_name = $_FILES['renewal_documents']['tmp_name'][$i];
        $type = $_FILES['renewal_documents']['type'][$i];
        $size = $_FILES['renewal_documents']['size'][$i];
        ...
        if (move_uploaded_file($tmp_name, $dest)) {
            $stmt = $conn->prepare("INSERT INTO highschool_renew_documents (account_id, file_name, file_path) VALUES (?, ?, ?)");
