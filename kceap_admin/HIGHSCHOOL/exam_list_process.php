        ...
        try {
            ...
            if ($duplicate_exists && $existingAccountId) {
                ...
                $update = $conn->prepare("
                    UPDATE highschool_account SET
                        ...
                        address = ?, phone_number = ?, schedule_date = ?, schedule_time = ?, status = ?, semester = ?, upload_deadline = ?
                    WHERE email = ?
                ");
                if ($update === false)
                    throw new Exception($conn->error);
                ...
                $insert = $conn->prepare("
                    ...
                    (applicant_id, first_name, middle_name, last_name, school, strand, year_level, address, phone_number, email, schedule_date, schedule_time, status, semester, upload_deadline) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                if ($insert === false)
                    throw new Exception($conn->error);
            ...
            try {
                ...
                $mail->Body = "
                    ...
                    <p>Congratulations! You have been accepted for the next step. Your status is now <b>{$status}</b>.</p>
