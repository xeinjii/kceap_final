...
$stmt = $conn->prepare("SELECT first_name, last_name, email FROM highschool_account WHERE applicant_id = ?");
...
try {
    // Update status to active and set semester to 1st SEMESTER
    $upd = $conn->prepare("UPDATE highschool_account SET status = 'active' WHERE applicant_id = ?");
    $upd->bind_param('i', $id);
    $upd->execute();
    $upd->close();

    // Get documents to delete files
    $docStmt = $conn->prepare("SELECT id, file_path FROM highschool_documents WHERE account_id = ?");
    ...
    $delDocs = $conn->prepare("DELETE FROM highschool_documents WHERE account_id = ?");
...
{
    $sql = "SELECT email, first_name, last_name FROM highschool_account WHERE applicant_id = ? LIMIT 1";
    $stmt = $conn->prepare($sql);
    if (!$stmt)
        return false;
    $stmt->bind_param("i", $applicant_id);
