...
if ($action === 'accept') {
    ...
    try {
        ...
        $update_sql = "UPDATE college_account SET status = 'active' WHERE applicant_id = ?";
        ...
        $delete_docs_sql = "DELETE FROM college_renew_documents WHERE account_id = ?";
        ...
        */
        ...
        $infoStmt = $conn->prepare("SELECT email, first_name, last_name FROM college_account WHERE applicant_id = ? LIMIT 1");
        if ($infoStmt) {
