...
if (isset($_GET['action']) && $_GET['action'] === 'check_deadline' && $email) {
    ...
    $stmt = $conn->prepare("SELECT upload_deadline, status, first_name, middle_name, last_name FROM college_account WHERE email = ? LIMIT 1");
    ...
    if (!empty($upload_deadline)) {
        ...
        if ($deadline instanceof DateTimeImmutable) {
            if ($now > $deadline && strtolower(trim($status)) !== 'expired') {
                // Update status in DB
                $upd = $conn->prepare("UPDATE college_account SET status='expired' WHERE email=?");
                if ($upd) {
...
<style>
body { font-family: 'Poppins', sans-serif; background: linear-gradient(135deg, #0d6efd, #0dcaf0); min-height: 100vh; padding-top: 80px; padding-bottom: 100px; }
.navbar { background: #0d6efd !important; }
.schedule-card { backdrop-filter: blur(15px); background: rgba(255,255,255,0.95); border-radius: 1.2rem; box-shadow: 0 8px 25px rgba(0,0,0,0.15); padding: 2rem; width: 95%; max-width: 900px; margin: auto; }
...
.footer-nav { position: fixed; bottom:0; left:0; width:100%; background:#fff; border-top:1px solid #ddd; display:flex; justify-content:space-around; padding:0.5rem 0; }
.footer-nav a { color:#0d6efd; text-decoration:none; display:flex; flex-direction:column; align-items:center; font-size:.75rem; }
.footer-nav a .material-symbols-outlined { font-size:1.5rem; }
