...
$account_id = $_SESSION['id'] ?? $_SESSION['college_user']['account_id'] ?? $_SESSION['account_id'] ?? $_POST['account_id'] ?? null;
...
$stmt = $conn->prepare("SELECT password FROM college_account WHERE id = ? LIMIT 1");
...
$upd = $conn->prepare("UPDATE college_account SET password = ? WHERE id = ?");
